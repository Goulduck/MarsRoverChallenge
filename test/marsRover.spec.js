...
describe('MarsRover', function () {
  ...
  describe('Moves in the correct direction', () => {
    it('Moves North', () => {
      const rover = new MarsRover([1, 2], 'N', [], [100, 100])
      rover.move()
      assert.deepEqual(rover.coordinates, [1, 3])
    })

    it('Moves East', () => {
      const rover = new MarsRover([1, 2], 'E', [], [100, 100])
      rover.move()
      assert.deepEqual(rover.coordinates, [2, 2])
    })

    it('Moves South', () => {
      const rover = new MarsRover([1, 2], 'S', [], [100, 100])
      rover.move()
      assert.deepEqual(rover.coordinates, [1, 1])
    })

    it('Moves West', () => {
      const rover = new MarsRover([1, 2], 'W', [], [100, 100])
      rover.move()
      assert.deepEqual(rover.coordinates, [0, 2])
    })

    it('Throws an error when move would result in collision', () => {
      const rover = new MarsRover([2, 5], 'E', [[3, 5], [1, 2]], [100, 100])
      assert.throws(() => rover.move(), Error, 'Co-ordinates result in collision')
    })
  })
  ...
  describe('Checks if next move is an obstacles', () => {
    ...
    it('Returns true if obstacle', () => {
      const rover = new MarsRover([1, 2], 'N', [[1, 3], [7, 9]], [100, 100])
